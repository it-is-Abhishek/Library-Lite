...
async function checkSeedData() {
  ...
  try {
    ...
    books.forEach(book => {
      const status = book.availableCopies === 0 ? '❌ All borrowed' : 
                     book.availableCopies < book.totalCopies ? `⚠️  ${book.availableCopies}/${book.totalCopies} available` :
    ...
    Object.entries(usersByRole).forEach(([role, roleUsers]) => {
      ...
      roleUsers.forEach(user => {
        const memberInfo = user.member ? ` (${user.member.membershipType} membership)` : '';
    ...
    if (reservations.length === 0) {
      ...
      reservations.forEach(reservation => {
        ...
        console.log(`- ${reservation.user.fullName} reserved "${reservation.book.title}"`);
        console.log(`  Reserved: ${reservation.reservedAt.toLocaleDateString()}`);
        console.log(`  Expires in: ${daysUntilExpiry} days`);
        console.log(`  Book availability: ${reservation.book.availableCopies} copies`);
      });
    }
    ...
    // Statistics
    console.log('\n=== Statistics ===');
    const borrowedLoans = await prisma.loan.count({ where: { status: 'borrowed' } });
